...
class bing_result extends search_engine {
    ...
    function loop_page($link) {
        ...
        while ($i <= ($this->cur_page + 70)) {
            $this->get_Result_Content($link . '&start=' . $i, array('bing', Check_Link::remove_http($this->domain)));
            $this->prt_file($i, 'bing');
            if ($this->end != '-1') {
                $i+=10;
            } else {
                $this->end = '-1';
